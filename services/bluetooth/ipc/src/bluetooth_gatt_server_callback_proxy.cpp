    ...
    const BluetoothGattDevice &device, const BluetoothGattCharacteristic &characteristic)
{
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_CHARACTERISTIC_READREQUEST, data, reply, option);
    if (error != NO_ERROR) {
...
void BluetoothGattServerCallbackProxy::OnConnectionStateChanged(const BluetoothGattDevice &device, int32_t ret,
    int32_t state, int32_t disconnectReason, std::string reasonMessage)
{
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_CONNECTIONSTATE_CHANGED, data, reply, option);
    if (error != NO_ERROR) {
...
void BluetoothGattServerCallbackProxy::OnAddService(int32_t ret, const BluetoothGattService &service)
{
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_ADD_SERVICE, data, reply, option);
    if (error != NO_ERROR) {
    ...
    const BluetoothGattDevice &device, const BluetoothGattDescriptor &descriptor)
{
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_DESCRIPTOR_READ_REQUEST, data, reply, option);
    if (error != NO_ERROR) {
    ...
    const BluetoothGattDevice &device, const BluetoothGattDescriptor &descriptor)
{
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_DESCRIPTOR_WRITE_REQUEST, data, reply, option);
    if (error != NO_ERROR) {
...
void BluetoothGattServerCallbackProxy::OnMtuChanged(const BluetoothGattDevice &device, int32_t mtu)
{
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_MTU_CHANGED, data, reply, option);
    if (error != NO_ERROR) {
    ...
    const BluetoothGattDevice &device, const BluetoothGattCharacteristic &characteristic, int result)
{
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_NOTIFY_CONFIRM, data, reply, option);
    if (error != NO_ERROR) {
...
void BluetoothGattServerCallbackProxy::OnBlePhyUpdate(
    const BluetoothGattDevice &device, int32_t txPhy, int32_t rxPhy, int32_t status)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(BluetoothGattServerCallbackProxy::GetDescriptor())) {
        HILOGE("BluetoothGattServerCallbackProxy::OnBlePhyUpdate WriteInterfaceToken error");
        return;
    }
    if (!data.WriteParcelable(&device)) {
        HILOGE("BluetoothGattServerCallbackProxy::write device error");
        return;
    }
    if (!data.WriteInt32(txPhy)) {
        HILOGE("BluetoothGattServerCallbackProxy::write txPhy error");
        return;
    }
    if (!data.WriteInt32(rxPhy)) {
        HILOGE("BluetoothGattServerCallbackProxy::write rxPhy error");
        return;
    }
    if (!data.WriteInt32(status)) {
        HILOGE("BluetoothGattServerCallbackProxy::write status error");
        return;
    }
    MessageParcel reply;
    MessageOption option {
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_BLE_PHY_UPDATE,
        data, reply, option);
    if (error != NO_ERROR) {
        HILOGE("BluetoothGattServerCallbackProxy::OnBlePhyUpdate done fail, error: %d", error);
        return;
    }
    return;
}

void BluetoothGattServerCallbackProxy::OnBlePhyRead(int32_t txPhy, int32_t rxPhy, int32_t status)
{
    MessageParcel data;
    if (!data.WriteInterfaceToken(BluetoothGattServerCallbackProxy::GetDescriptor())) {
        HILOGE("BluetoothGattServerCallbackProxy::OnBlePhyRead WriteInterfaceToken error");
        return;
    }
    if (!data.WriteInt32(txPhy)) {
        HILOGE("BluetoothGattServerCallbackProxy::write txPhy error");
    ...
    int error = Remote()->SendRequest(
        BluetoothGattServerCallbackInterfaceCode::GATT_SERVER_CALLBACK_BLE_PHY_READ,
        data, reply, option);
    if (error != NO_ERROR) {
        HILOGE("BluetoothGattServerCallbackProxy::OnBlePhyRead done fail, error: %d", error);
        return;
    }
    return;
}
