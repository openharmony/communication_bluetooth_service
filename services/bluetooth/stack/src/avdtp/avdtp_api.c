...
uint16_t AvdtSetConfigRsp(uint16_t handle, uint8_t transLabel, AvdtCatetory category)
{
    ...
    if (streamCtrl != NULL) {
        ...
        LOG_ERROR("[AVDT]%{public}s: AvdtGetStreamCtrlByHandle Failed,input parameter invalid handl(%hu)",
            __func__, handle);
    }
    return Ret;
}
...
{
    LOG_INFO("[AVDT]%{public}s: addr(%02x:%02x:%02x:%02x:%02x:%02x),pkrole(%hhu)",
        __func__, BT_ADDR_FMT_DSC(bdAddr->addr), role);
    uint16_t Ret = AVDT_FAILED;
    Event *event = EventCreate(true);
...
{
    LOG_DEBUG("[AVDT]%{public}s: addr(%02x:%02x:%02x:%02x:%02x:%02x),pkrole(%hhu)",
        __func__, BT_ADDR_FMT_DSC(bdAddr->addr), role);
    uint16_t Ret = AVDT_SUCCESS;
