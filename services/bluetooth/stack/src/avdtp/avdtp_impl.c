    ...
    for (int i = 0; i < AVDT_NUM_SEPS; i++) {
        if (sigCtrl->streamCtrl[i].isAllocated) {
            ...
            if (streamConfig != NULL) {
                ...
                LOG_DEBUG("[AVDT]%{public}s: Failed to get SEP: index(%hu)",
                    __func__, sigCtrl->streamCtrl[i].codecIndex);
            }
        }
    }
...
AvdtSepConfig AvdtGetConfig(AvdtSigCtrl *sigCtrl, uint16_t handle)
{
    ...
    for (int i = 0; i < AVDT_NUM_SEPS; i++) {
        if (sigCtrl->streamCtrl[i].isAllocated && sigCtrl->streamCtrl[i].handle == handle) {
            ...
            LOG_DEBUG("[AVDT]%{public}s: sepNumber(%{public}d),seid(%hu),capability.pscMask(%hu),"
                "capability.numCodec(%hhu)",
                __func__,
                i,
                handle,
                capability.pscMask,
