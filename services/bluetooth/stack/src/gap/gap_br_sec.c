...
int GAP_RequestSecurity(const BtAddr *addr, const GapRequestSecurityParam *param)
{
    ...
    do {
        ...
        if (node == NULL) {
            LOG_ERROR("%{public}s:Register(%{public}d, %{public}d)not found.",
                __FUNCTION__, param->info.serviceId, param->info.direction);
...
int GAP_AuthorizeRes(const BtAddr *addr, GAP_Service service, uint8_t accept)
{
    ...
    LOG_INFO("%{public}s:" BT_ADDR_FMT " %{public}d, accept[%hhu]",
        __FUNCTION__, BT_ADDR_FMT_OUTPUT(addr->addr), service, accept);
...
void GapIOCapabilityRequestReplyComplete(const HciIOCapabilityRequestReplyReturnParam *param)
{
    if (param->status != HCI_SUCCESS) {
        ...
        LOG_WARN("%{public}s:" BT_ADDR_FMT "Failed: %02x",
            __FUNCTION__, BT_ADDR_FMT_OUTPUT(param->bdAddr.raw), param->status);
        GapIOCapabilityRequestNegativeReply(&addr, HCI_PAIRING_NOT_ALLOWED);
    }
}
...
{
    if (param->status != HCI_SUCCESS) {
        LOG_WARN("%{public}s:" BT_ADDR_FMT "Failed:%02x", __FUNCTION__,
            BT_ADDR_FMT_OUTPUT(param->bdAddr.raw), param->status);
    }
}
...
int GAP_LinkKeyRsp(const BtAddr *addr, uint8_t accept, const uint8_t linkKey[GAP_LINKKEY_SIZE], uint8_t keyType)
{
    ...
    LOG_INFO("%{public}s:" BT_ADDR_FMT " %hhu, keyType[%hhu]",
        __FUNCTION__, BT_ADDR_FMT_OUTPUT(addr->addr), accept, keyType);
