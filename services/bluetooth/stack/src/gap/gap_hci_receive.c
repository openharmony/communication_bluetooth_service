...
static void GapRecvLeEnhancedReadTransmitPowerLevelComplete(const HciLeEnhancedReadTransmitPowerLevelReturnParam *param)
{
    if (param == NULL) {
        return;
    }

    HILOGI("status: 0x%{public}02x, handle: 0x%{public}04x, phy: 0x%{public}02x", param->status,
        param->connectionHandle, param->phy);
    int ret =
        GapProcessHciEventInTask((TaskFunc)GapLeEnhancedReadTransmitPowerLevelComplete, param, sizeof(*param), NULL);
    if (ret != BT_SUCCESS) {
        HILOGE("Task error: %{public}d.", ret);
    }
}

static void GapRecvLeSetPathLossReportingParametersComplete(const HciLeSetPathLossReportingParametersReturnParam *param)
{
    if (param == NULL) {
        return;
    }
    ...
    HILOGI("status: 0x%{public}02x, handle: 0x%{public}04x", param->status, param->connectionHandle);
    int ret =
        GapProcessHciEventInTask((TaskFunc)GapLeSetPathLossReportingParametersComplete, param, sizeof(*param), NULL);
    if (ret != BT_SUCCESS) {
        HILOGE("Task error: %{public}d.", ret);
    }
}

static void GapRecvLeSetPathLossReportingEnableComplete(const HciLeSetPathLossReportingEnableReturnParam *param)
{
    if (param == NULL) {
        return;
    }
    ...
    HILOGI("status: 0x%{public}02x, handle: 0x%{public}04x", param->status, param->connectionHandle);
    int ret = GapProcessHciEventInTask((TaskFunc)GapLeSetPathLossReportingEnableComplete, param, sizeof(*param), NULL);
    if (ret != BT_SUCCESS) {
        HILOGE("Task error: %{public}d.", ret);
    }
}

static void GapRecvLeSetTransmitPowerReportingEnableComplete(
    const HciLeSetTransmitPowerReportingEnableReturnParam *param)
{
    if (param == NULL) {
        return;
    }
    ...
    HILOGI("status: 0x%{public}02x, handle: 0x%{public}04x", param->status, param->connectionHandle);
    int ret =
        GapProcessHciEventInTask((TaskFunc)GapLeSetTransmitPowerReportingEnableComplete, param, sizeof(*param), NULL);
    if (ret != BT_SUCCESS) {
        HILOGE("Task error: %{public}d.", ret);
    }
}
...
static void GapRecvLeTransmitPowerReportingEvent(const HciLeTransmitPowerReportingEventParam *eventParam)
{
    ...
    HILOGI("status: 0x%{public}02x, handle: 0x%{public}04x, reason: 0x%{public}02x", eventParam->status,
        eventParam->connectionHandle, eventParam->reason);
    int ret =
        GapProcessHciEventInTask((TaskFunc)GapLeTransmitPowerReportingEvent, eventParam, sizeof(*eventParam), NULL);
    if (ret != BT_SUCCESS) {
        HILOGE("Task error: %{public}d.", ret);
    }
}

static void GapRecvLePathLossThresholdEvent(const HciLePathLossThresholdEventParam *eventParam)
{
    if (eventParam == NULL) {
        return;
    }

    HILOGI("handle: 0x%{public}04x, currentPathLoss: %{public}u, zoneEntered: 0x%{public}02x",
        eventParam->connectionHandle, eventParam->currentPathLoss, eventParam->zoneEntered);
    int ret = GapProcessHciEventInTask((TaskFunc)GapLePathLossThresholdEvent, eventParam, sizeof(*eventParam), NULL);
    if (ret != BT_SUCCESS) {
        HILOGE("Task error: %{public}d.", ret);
    }
}
...
static HciEventCallbacks g_hciEventCallbacks = {
    ...
    .leEnhancedReadTransmitPowerLevelComplete = GapRecvLeEnhancedReadTransmitPowerLevelComplete,
