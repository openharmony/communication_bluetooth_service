...
{
    OBEX_LOG_DEBUG("ObexHeader Parse: isRequest[%{public}d],isConnect:[%{public}d], buf size=[%{public}d]",
        isRequest, isConnect, size);
    ...
    if (packetLength_ != packetLengthForCheck) {
        OBEX_LOG_ERROR(
            "parse error packetLength_[%{public}d] != packetLengthForCheck[%{public}d]",
            packetLength_, packetLengthForCheck);
