...
void HfpHfCommandProcessor::ProcessCindTester(HfpHfDataConnection &dataConn, std::string arg)
{
    ...
    for (; ;) {
        ...
        LOG_DEBUG("[HFP HF]%{public}s():name[%{public}s], min[%u], max[%u], offset[%{public}d], res[%{public}d]",
            __FUNCTION__, name, min, max, offset, res);
        if (res != CHLD_TEST_ARGS_NUMBER) {
            LOG_WARN("[HFP HF]%{public}s():invalid command name[%{public}s], min[%u], max[%u], offset[%{public}d], "
                "res[%{public}d]", __FUNCTION__, name, min, max, offset, res);
            return;
        }
...
void HfpHfCommandProcessor::ConnectSlc(HfpHfDataConnection &dataConn)
{
    ...
    switch (currentCommand_) {
        ...
        case AT_BIND_GETTER:
            ...
            break;
        default:
            LOG_ERROR("[HFP HF]%{public}s():invalid commandId[%{public}d] in slc connection command",
                __FUNCTION__, currentCommand_);
    ...
    HfpHfDataConnection &dataConn, const std::string &command, int commandId)
{
    ...
    if (currentCommand_ == AT_COMMAND_NONE || !dataConn.slcConnected_ || (hspState_ == HSP_HS_STATE_HSP)) {
        LOG_DEBUG("[HFP HF]%{public}s(): commandId[%{public}d], command[%{public}s]",
            __FUNCTION__, commandId, command.c_str());
...
void HfpHfCommandProcessor::SendQueuedAtCommand(HfpHfDataConnection &dataConn)
{
    ...
    commandQueue_.pop();
    LOG_DEBUG(
        "[HFP HF]%{public}s() commandId[%{public}d], command[%{public}s]",
            __FUNCTION__, std::get<1>(command), std::get<0>(command).c_str());
