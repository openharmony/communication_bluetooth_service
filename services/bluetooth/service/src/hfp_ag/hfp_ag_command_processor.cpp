...
int HfpAgCommandProcessor::StoiTryCatch(HfpAgDataConnection &dataConn, const std::string &arg)
{
    ...
    {
        LOG_ERROR("[HFP AG]%{public}s():invalid numeric argument", __FUNCTION__);
...
void HfpAgCommandProcessor::BacSetter(HfpAgDataConnection &dataConn, const std::string &arg)
{
    ...
    if (currentRemoteCodec != dataConn.remoteSupportCodecs_) {
        LOG_INFO("[HFP AG]%{public}s():Remote support codecs updated, currentRemoteCodec[%{public}d], "
            "remoteSupportCodecs_[%{public}d]", __FUNCTION__, currentRemoteCodec, dataConn.remoteSupportCodecs_);
