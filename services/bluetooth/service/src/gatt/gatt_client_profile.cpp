...
{
    LOG_INFO("%{public}s: connectHandle is %hu, Add requestList_: DISCOVER_ALL_PRIMARY_SERVICE.",
...
void GattClientProfile::impl::SplitReadByTypeRsp(
    uint16_t connectHandle, AttEventData *data, std::list<std::pair<uint16_t, GattRequestInfo>>::iterator iter)
{
    LOG_INFO("%{public}s: connectHandle is %hu, respType is %{public}d.",
        __FUNCTION__, connectHandle, iter->second.reqType_);
    ...
    uint16_t connectHandle, std::list<std::pair<uint16_t, GattRequestInfo>>::iterator iter)
{
    ...
    switch (iter->second.reqType_) {
        ...
        case FIND_INCLUDE_SERVICE:
            ...
            break;
        default:
            LOG_ERROR("%{public}s: Response type is %{public}d. It's invalid type.",
                __FUNCTION__, iter->second.reqType_);
            break;
    }
}
